...
void encryptDecrypt(const bool isEncrypt, char *key, char *inputText, char *outputText)
{
    ...
    for (inputTextIndex = 0; inputText[inputTextIndex] != '\0'; inputTextIndex++)
    {
        ...
        // The encryption equation:- (Pi + Ki) mod 26
        ...
        // The decryption equation:- (Ci - Ki) mod 26
        ...
        if (isEncrypt)
        {
            processedText = (messageToProcess + keyIndex) % 26;
            key = processedText; // Update the key
        }
        else
        {
            processedText = (messageToProcess - keyIndex + 26) % 26;
